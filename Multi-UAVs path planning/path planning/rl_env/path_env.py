...
class RlGame(gym.Env):
    def __init__(self, n,m,render=False):
        ...
        if self.Render:
            ...
            self.GRAPHICS = tools.load_graphics('/home/lwh/UAV-path-planning/Multi-UAVs path planning/path planning/assignment/source/image')

            self.SOUND = tools.load_sound('/home/lwh/UAV-path-planning/Multi-UAVs path planning/path planning/assignment/source/music')
    ...
    def reset(self):#reset的仅是环境状态，
        ...
        return np.array(obs_list)#np.array([self.my_game.state.hero['hero0'].posx/1000,self.my_game.state.hero['hero0'].posy/1000,self.my_game.state.hero['hero0'].speed/2,self.my_game.state.hero['hero0'].theta*57.3/360])#np.zeros((self.n,2)).flatten()
